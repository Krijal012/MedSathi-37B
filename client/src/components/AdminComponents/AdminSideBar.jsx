...
const AdminSidebar = ({ isOpen, toggleSidebar }) => {
    const menuItems = [
        ...
        { name: 'Manage Patients', path: '/manage-patients', icon: '👥' },
        { name: 'Manage Staffs', path: '/manage-staffs', icon: '👨‍⚕️' },
    ];
    ...
    return (
        <>
            ...
            <div className={`fixed top-0 left-0 h-full w-64 bg-gray-800 text-white flex flex-col z-40 transform ${isOpen ? 'translate-x-0' : '-translate-x-full'} transition-transform duration-300 ease-in-out lg:translate-x-0 lg:fixed`}>
                {/* Logo */}
                <Link to="/admin-dashboard" className="p-6 flex items-center space-x-3 border-b border-gray-700 hover:bg-gray-700 transition-colors">
